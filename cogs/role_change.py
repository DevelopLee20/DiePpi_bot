...
from core.enums import Role, Mode
...
class RoleChange(commands.Cog):
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        total_study_min = await StudyCollection.find_total_study_min_in_today(str(member.id))
        
        if total_study_min > 180:
            role = discord.utils.get(guild.roles, name=Role.DEVELOPMENT_FAIRY.value)  # 개발 요정 역할
            if role and role not in member.roles:
                await member.add_roles(role)
                await alert_channel.send(upgrade_role_message(member.mention, role.name))
